            ...
            <form action="<?= $contador ? '/contadores/actualizar/' . $contador['id'] : '/contadores/crear' ?>" method="post">
                    ...
                    <select id="cliente_id" name="cliente_id" class="form-select" required>
                        ...
                        <?php foreach ($clientes as $cliente): ?>
                            <option value="<?= $cliente['id'] ?>"
                                <?= (isset($contador['cliente_id']) && $contador['cliente_id'] == $cliente['id']) ? 'selected' : '' ?>>
                                <?= esc($cliente['nombre']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    ...
                    <select id="tipo_servicio_id" name="tipo_servicio_id" class="form-select" required>
                        ...
                        <?php foreach ($tiposServicio as $tipo): ?>
                            <option value="<?= $tipo['id'] ?>"
                                <?= (isset($contador['tipo_servicio_id']) && $contador['tipo_servicio_id'] == $tipo['id']) ? 'selected' : '' ?>>
                                <?= esc($tipo['nombre']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input id="codigo" type="text" name="codigo" class="form-control" maxlength="20"
                           value="<?= esc($contador['codigo'] ?? old('codigo')) ?>" required>
